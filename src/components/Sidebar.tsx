...
const Sidebar = ({ open = false, onClose, onCreate }: { open?: boolean; onClose?: () => void; onCreate?: () => void }) => {
  return (
    <>
      {/* Desktop sidebar */}
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 bg-[#142D52] text-white p-4">
            ...
            {/* Sticky footer inside mobile sidebar: create button + image */}
            <div className="absolute bottom-0 left-0 w-full p-4 border-t border-[#06BF97] bg-[#142D52]">
              <button
                onClick={() => { onClose && onClose(); /* navigate to create */ window.location.href = '/create'; }}
                className="w-full py-3 rounded-md bg-emerald-500 text-white text-sm font-medium hover:bg-emerald-600"
              >
                Create New Recruitment
              </button>

              <div className="mt-3 flex justify-center">
                <Image src="/sideBarImage.svg" alt="Sidebar Image" height={110} width={110} />
              </div>
            </div>
          </div>
        </div>
